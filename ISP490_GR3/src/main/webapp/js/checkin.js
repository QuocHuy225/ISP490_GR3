...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (updateAppointmentModalEl) {
        updateAppointmentModalEl.addEventListener('show.bs.modal', function (event) {
            ...
            if (modalTitle)
                modalTitle.textContent = `Cập nhật lịch hẹn ID: ${id}`;
            if (updateAppointmentIdInput)
                updateAppointmentIdInput.value = id;
            if (updateAppointmentCodeInput)
                updateAppointmentCodeInput.value = code;
            if (updatePatientIdInput)
                updatePatientIdInput.value = patientId;
            if (updateDoctorIdInput)
                updateDoctorIdInput.value = doctorId;
            if (updateSlotIdInput)
                updateSlotIdInput.value = slotId;
